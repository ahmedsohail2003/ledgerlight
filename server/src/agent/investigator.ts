...
export function buildPrompt(target: string, pack: EvidencePack, corrections?: string[]): string {
  const parts = [
    ...
    '- The headline and limitations are validated for numbers exactly like claim text: any number there must match a figure declared in some claim, so prefer number-free headlines. In limitations, quote data_coverage_note verbatim if you reference it.',
    '- Scale shorthand only passes validation with the unit word present: write "$19.1 million", never a bare "19.1", when the declared figure is 19100000.',
    '- overall_assessment must agree with fired_rules: indicators_warrant_review only if at least one rule fired; no_indicators_found only if none did.',
